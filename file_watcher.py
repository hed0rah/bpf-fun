#!/usr/bin/env python3
"""BPF program that watches file opens, reads, and writes.
...
Usage:
    ...
    sudo python3 file_watcher.py python3                # filter by comm name
    sudo python3 file_watcher.py --pid 1234             # filter by pid
    sudo python3 file_watcher.py --path /var/log        # only files under path
    sudo python3 file_watcher.py --summary 10           # summary every 10s
    sudo python3 file_watcher.py --top 20               # top 20 in summary
    sudo python3 file_watcher.py --quiet                # summary only, no live events
    sudo python3 file_watcher.py --sort ops             # sort by operation count

Examples:

    # What's thrashing the disk on a busy server? Summary-only, sorted by bytes:
    sudo python3 file_watcher.py --quiet --summary 10

    # Find which process keeps writing to /var/log and how much:
    sudo python3 file_watcher.py --path /var/log --quiet --sort bytes

    # App is doing a ton of small reads -- find the inode-heavy offender:
    sudo python3 file_watcher.py java --sort ops --summary 5

    # Watch what files nginx touches in real time:
    sudo python3 file_watcher.py nginx

    # Quick check -- what's a specific PID doing right now?
    sudo python3 file_watcher.py --pid 1234

    # Production: top 30 files hit in the last 30s, no live noise:
    sudo python3 file_watcher.py --quiet --summary 30 --top 30

    # Investigate a DB -- is postgres reading from disk or cache?
    sudo python3 file_watcher.py postgres --path /var/lib/postgresql --summary 5

    # Catch config file reads across all processes:
    sudo python3 file_watcher.py --path /etc --sort ops --quiet

    # Ignore noisy background processes:
    sudo python3 file_watcher.py --ignore 'bash|sshd|grep|cat|systemd'

    # Combine -- watch /var/log but ignore known log rotators:
    sudo python3 file_watcher.py --path /var/log --ignore 'logrotate|gzip' --quiet
"""
...
parser = argparse.ArgumentParser(
    description="Watch file I/O with periodic summary of top talkers")
...
print(f"Watching file I/O: {filter_desc}  (self pid={my_pid} excluded)")
if args.summary > 0:
